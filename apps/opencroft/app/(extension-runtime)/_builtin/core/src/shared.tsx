...
export function PinButton({ icon: Icon, label, onClick }: PinButtonProps) {
  return (
    <Button variant='ghost' size='sm' className='nodrag nopan h-5 text-[10px] px-1.5' onClick={onClick}>
...
export function StatsList({
  ...
  items: { icon: React.ComponentType<{ className?: string }>; value: string }[]
}) {
  return (
...
export function PinnedBody({ input, output }: { input?: React.ReactNode; output?: React.ReactNode }) {
  return (
    ...
    <>
      <NodeResizer
        ...
        onResizeEnd={(_event, { width, height }) => {
          setNodes((nds) =>
            nds.map((n) =>
              n.id === id ? { ...n, style: { ...n.style, width: snap(width), height: snap(height) } } : n,
            ),
          )
        }}
      ...
      <div className={`h-full w-full ${closing ? 'window-node-close' : 'window-node-open'}`}>
        <NodeCard selected={selected} loading={loading} accent={accent} className='h-full w-full min-w-0 flex flex-col'>
          <div className='flex items-center gap-2 px-4 py-2'>
            {input}
            <Icon className={`h-4 w-4 shrink-0 ${iconClassName ?? ''}`} />
            <span className='text-sm font-medium truncate flex-1'>{title}</span>
            <Button variant='ghost' size='icon' className='nodrag nopan h-4 w-4 shrink-0' onClick={close}>
