...
import { useTerminalWs } from '@/app/(terminal)/_hooks/use-terminal-ws'
import { terminalTheme } from '@/app/(terminal)/_lib/theme'
...
export function TerminalView({ termConfig, onConnected, onDisconnected, onError }: TerminalViewProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([import('@xterm/xterm'), import('@xterm/addon-fit')]).then(([xtermMod, fitMod]) => {
