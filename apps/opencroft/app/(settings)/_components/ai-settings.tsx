  ...
  return (
    <div className='p-6 space-y-8'>
      ...
      <section className='space-y-2 max-w-md'>
        ...
        <p className='text-sm text-muted-foreground'>Used for messages sent from the command bar.</p>
        {state.connection.status === 'not-configured' ? (
          <p className='text-xs text-muted-foreground'>Configure the gateway above to load agents.</p>
          ...
          <>
            ...
            {state.agents.length === 0 && <p className='text-xs text-muted-foreground'>No agents available.</p>}
          </>
        )}
      </section>
    </div>
