...
export const deleteSession = createServerFn({ method: 'POST', strict: { output: false } })
...
export const loadSession = createServerFn({ method: 'POST', strict: { output: false } })
  .inputValidator((key: string) => key)
  .handler(async ({ data: key }): Promise<RawChatMessage[]> => {
