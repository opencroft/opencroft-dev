...
import { ArrowLeft, ChevronDown, FileText, History, Loader2, Menu, Pencil, Plus, Search, Trash2, X } from 'lucide-react'
...
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle } from '@opencroft/ui-kit/alert-dialog'
...
import { ScrollHeader } from '@opencroft/ui-kit/layout/scrollpage'
...
function moveIndicator(container: Element | null, indicator: Element | null, activeEl: Element | null) {
        ...
        return (
          <li key={entry.path} className='group/file relative'>
            ...
            >
              ...
              {changedFiles?.has(entry.path) && <Pencil className='size-3 shrink-0 text-orange-500 ml-auto mr-6' />}
            </a>
            <button
              onClick={(e) => {
                e.preventDefault()
  ...
  return (
    <div className='prose-docs' ref={ref}>
      <ReactMarkdown
        ...
        components={{
          h2: ({ children, ...p }) => (
            <h2 id={slugify(children)} {...p}>
              {children}
            </h2>
          ),
          h3: ({ children, ...p }) => (
            <h3 id={slugify(children)} {...p}>
              {children}
            </h3>
          ),
...
export default function DocsPage() {
  ...
  const loadDoc = useCallback(
    async (p: string) => {
      ...
      setLoading(false)
      if (namespace) {
        getGitFileLog({ data: { namespace, filePath: p } })
          .then((log) => setFileLog(log))
          .catch(() => {})
      }
    },
  ...
  const handleTocClick = useCallback(
    (id: string) => {
      ...
      const animate = (timestamp: number) => {
        ...
        const eased = 1 - (1 - progress) ** 5 // ease-out quint
  ...
  const sidebarNav = searchQuery.trim() ? (
    <DocSearchResultsList results={searchResults} loading={searching} onSelect={handleSearchSelect} />
  ) : (
    <SidebarNav
    ...
    <div className='flex-1 min-h-0'>
      ...
      <div className='hidden md:flex h-full min-h-0'>
        <div className='mx-auto flex h-full min-h-0 max-w-7xl'>
          ...
          <div className='flex-1 min-h-0'>
            <ScrollArea className='h-full'>
              <div className='py-6 px-8 mx-auto w-4xl'>{contentEl}</div>
            </ScrollArea>
          </div>
      ...
      <div className='md:hidden flex-1 min-h-0 flex flex-col'>
        ...
        <ScrollArea className='flex-1 min-h-0'>
          <div className='px-4 py-4 min-w-0'>{contentEl}</div>
        </ScrollArea>
      </div>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreate} disabled={!createPath.trim()}>
              Create
            </Button>
          </DialogFooter>
        </DialogContent>
