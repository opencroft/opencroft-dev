...
export function UrlMenuLayout({ param = 'section', defaultValue = '', children, ...props }: UrlMenuLayoutProps) {
  ...
  const sections = allChildren.filter((child): child is React.ReactElement<MenuContentProps | MenuPageProps> => React.isValidElement(child) && (child.type === MenuContent || child.type === MenuPage))

  const footers = allChildren.filter((child): child is React.ReactElement<MenuFooterProps> => React.isValidElement(child) && child.type === MenuFooter)
      ...
      {sections.map(({ props: s }) => {
        ...
        return (
          <button
            ...
            className={cn('w-full flex items-center gap-2 px-3 py-2 rounded-md text-sm transition-colors', value === s.id ? 'bg-accent font-medium' : 'hover:bg-accent/50')}
          >
