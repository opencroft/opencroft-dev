...
export function StickySection({
  ...
  const content =
    variant !== 'ghost' ? (
      <Flex row={!horizontal} className={variantClasses[variant]}>
        {children}
      </Flex>
    ) : (
      children
  ...
  return (
    <Flex row={horizontal} withSpacing className={cn('sticky z-1', sideClasses[side], className)} {...props}>
      {fade && <div className={cn('absolute pointer-events-none -z-1', fadeClasses[side])} />}
      {content}
    </Flex>
  )
}
