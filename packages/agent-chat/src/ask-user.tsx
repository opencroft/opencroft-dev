...
export interface AskUserProps {
  ...
  // Receives `{ [question.title]: answer }` once the user submits.
  onSubmit: (answers: Record<string, string>) => void
  onCancel?: () => void
  // Disables actions while an answer/cancel is in flight (host-controlled).
  pending?: boolean
  // Reports the active question index, e.g. for an external progress label.
...
// A self-contained, tabbed multi-question prompt: single/multi-select options
// plus custom answers, advancing to a final submit. Fully controlled — the host
// owns persistence (`onSubmit` / `onCancel`) and the `pending` flag.
export function AskUser({ questions, onSubmit, onCancel, pending = false, onCurrentChange }: AskUserProps) {
  ...
  // Commit pending custom text for the current question into answers.
  ...
  // Commit pending text, then advance to the next tab or submit.
