...
import { ScrollContent, ScrollFooter, ScrollPage } from '@/components/ui/layout/scrollpage';
...
export function UrlMenuLayout({ param = 'section', defaultValue = '', children, ...props }: UrlMenuLayoutProps) {
  ...
  const footers = allChildren.filter(
    (child): child is React.ReactElement<MenuFooterProps> =>
      React.isValidElement(child) && child.type === MenuFooter
  );
  ...
  const pageContent = activeSection?.type === MenuPage
    ...
    : (
      <ScrollPage>
        ...
        {activeFooter && (
          <ScrollFooter className="justify-end px-4">
            {activeFooter.props.children}
          </ScrollFooter>
        )}
      </ScrollPage>
    );
