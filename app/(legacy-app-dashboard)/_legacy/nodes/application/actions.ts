...
export const saveApp = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<void> => {
    ...
    const compose: Record<string, unknown> = {
      services: Object.fromEntries(appData.services.map((s) => [s.name, serviceToYaml(s)])),
    }
...
export const getContainerStatuses = createServerFn({ method: 'POST' })
  .inputValidator((appId: string) => appId)
  .handler(async ({ data: appId }): Promise<Record<string, string>> => {
...
export const composeUp = createServerFn({ method: 'POST' })
  .inputValidator((appId: string) => appId)
  .handler(async ({ data: appId }): Promise<void> => {
    const file = composeFile(appId)
    const { execFile } = await import('node:child_process')
...
export const composeDown = createServerFn({ method: 'POST' })
  .inputValidator((appId: string) => appId)
  .handler(async ({ data: appId }): Promise<void> => {
    const file = composeFile(appId)
    const { execFile } = await import('node:child_process')
