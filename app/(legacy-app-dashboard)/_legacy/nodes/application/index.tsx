      ...
      {service.ports.map((p, i) => (
        <div key={i} className='flex items-center gap-1'>
          <Input
            value={p.host}
            onChange={(e) => {
              const next = [...service.ports]
              next[i] = { ...next[i], host: e.target.value }
              onChange({ ...service, ports: next })
            }}
            placeholder='host'
      ...
      {service.volumes.map((v, i) => (
        <div key={i} className='flex items-center gap-1'>
          <Input
            value={v.host}
            onChange={(e) => {
              const next = [...service.volumes]
              next[i] = { ...next[i], host: e.target.value }
              onChange({ ...service, volumes: next })
            }}
...
function ApplicationSettings({ id, updateData, onDirtyChange, onLoadingChange }: NodeSettingsProps<ApplicationNodeData>) {
  ...
  const updateService = useCallback(
    (index: number, service: AppService) => {
      setAppData((prev) => {
