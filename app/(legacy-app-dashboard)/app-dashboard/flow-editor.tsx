...
import { loadGraph, saveGraph } from '@/app/(legacy-app-dashboard)/app-dashboard/actions';
import { CommentNode } from '@/app/(legacy-app-dashboard)/app-dashboard/bubble-hint-node';
import { FlowContextMenu } from '@/app/(legacy-app-dashboard)/app-dashboard/context-menu';
import { useCustomTemplates } from '@/app/(legacy-app-dashboard)/app-dashboard/custom-templates-context';
import '@/app/(legacy-app-dashboard)/app-dashboard/flow-editor.css';
import { useGraphEvents } from '@/app/(legacy-app-dashboard)/app-dashboard/hooks/use-graph-events';
import { NodeSettingsPanel } from '@/app/(legacy-app-dashboard)/app-dashboard/node-settings-panel';
import { buildNodeTypes } from '@/app/(legacy-app-dashboard)/app-dashboard/registry';
import { TemplateEditorDialog } from '@/app/(legacy-app-dashboard)/app-dashboard/template-editor-dialog';
import { nodeDefinitions } from '@/app/(legacy-app-dashboard)/nodes';
import { type CustomTemplate } from '@/app/(legacy-app-dashboard)/nodes/custom/types';
import { WindowNodeComponent } from '@/app/(legacy-app-dashboard)/nodes/window';
...
export function FlowEditor() {
  ...
  const allDefinitions = useMemo(
    () => [...nodeDefinitions, ...customDefinitions],
    [customDefinitions],
  );
  ...
  const scheduleAutoSave = useCallback((n: Node[], e: Edge[]) => {
    if (loaded) {
      debouncedSave(n, e);
    }
  ...
  const onNodeDragStart = useCallback((_event: React.MouseEvent, node: Node) => {
    ...
    for (const n of nodes) {
      ...
      if (n.position.x >= node.position.x && n.position.x + nw <= node.position.x + sw &&
          n.position.y >= node.position.y && n.position.y + nh <= node.position.y + sh) {
        childIds.add(n.id);
      }
    }
      ...
      <div className="flex-1">
        ...
        {menu && (
          <FlowContextMenu
            ...
            onSelect={onAddNode}
            onNewCustomType={() => {
              setEditingTemplate(null); setMenu(null);
            }}
            onClose={closeMenu}
          />
        )}
