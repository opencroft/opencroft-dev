...
export function ButtonPin({ handleId, icon: Icon, label, side, onClick }: ButtonPinProps) {
  ...
  return (
    <Button variant="ghost" size="sm" className="nodrag nopan h-5 text-[10px] px-1.5 w-full justify-between" onClick={onClick}>
      {side === 'left' && (
        <Handle type={type} position={pos} id={handleId} className="inline-handle" />
      )}
