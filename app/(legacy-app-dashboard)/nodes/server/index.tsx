...
import { useSettingsDraft } from '@/app/(legacy-app-dashboard)/app-dashboard/use-settings-draft';
import { type KeyEntry, listKeys } from '@/app/(legacy-app-dashboard)/nodes/key-store/actions';
import { applyServerConfig, resolveServer } from '@/app/(legacy-app-dashboard)/nodes/server/actions';
import { ButtonPin, HANDLE_EXECUTION, HANDLE_FILESYSTEM } from '@/app/(legacy-app-dashboard)/nodes/shared/button-pin';
import { PinnedNode, StatsList } from '@/app/(legacy-app-dashboard)/nodes/shared/pinned-node';
import { spawnFileBrowserWindow, spawnTerminalWindow } from '@/app/(legacy-app-dashboard)/nodes/shared/spawn-window';
import { checkDocker, getServerStats, installDockerUbuntu, type ServerStats } from '@/app/(server)/server/remote';
import { type DockerFeature, type ServerFeature, type SshFeature, slug } from '@/app/(server)/server/types';
...
function buildTerminalConfig(data: ServerData): import('@/app/(terminal)/terminal/types').TerminalConfig | null {
...
function ServerComponent({ data, selected, positionAbsoluteX, positionAbsoluteY }: NodeProps<ServerNode>) {
  ...
  const openTerminal = useCallback(async () => {
    ...
    setNodes((nds) => [...nds, spawnTerminalWindow(
  ...
  const openFiles = useCallback(async () => {
    ...
    setNodes((nds) => [...nds, spawnFileBrowserWindow(
      { title: data.name, x: positionAbsoluteX, y: positionAbsoluteY },
      {
  ...
  const status = loading ? 'warning' as const
    : error ? 'destructive' as const
      : stats ? 'success' as const
        : undefined;
...
function DockerSubSection({ feature, serverData, onUpdate }: {
  ...
  const StatusIcon = feature.installed === true ? Check
    : feature.installed === false ? X : null;

  const statusText = feature.installed === undefined ? 'Status unknown'
    : feature.installed ? 'Docker is installed' : 'Docker not installed';
      ...
      <Flex row className="gap-1">
        ...
        {feature.installed === false && (
          <Button variant="outline" size="sm" className="h-6 text-[10px]" onClick={handleInstall} disabled={busy}>
            {installing ? <Spinner className="h-3 w-3" /> : <><Download className="h-3 w-3 mr-0.5" />Install (Ubuntu)</>}
          </Button>
        )}
      </Flex>
    </>
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-col gap-1">
        ...
        {draft.name && (
          <span className="text-[10px] text-muted-foreground">{slug(draft.name)}</span>
        )}
      </div>
      <div className="flex flex-col gap-1">
