    ...
    return new Promise<void>((resolve, reject) => {
      ...
      xhr.upload.onprogress = (e) => {
        if (e.lengthComputable) {
          ...
          lastLoaded = e.loaded
          lastTime = now
          setUploads((prev) => prev.map((u) => (u.id === entry.id ? { ...u, loaded: e.loaded, speed } : u)))
        }
      }

      xhr.onload = () => {
        xhrMap.current.delete(entry.id)
        setUploads((prev) => prev.map((u) => (u.id === entry.id ? { ...u, loaded: u.total, done: true, speed: 0 } : u)))
      ...
      xhr.onabort = () => {
        xhrMap.current.delete(entry.id)
        setUploads((prev) => prev.map((u) => (u.id === entry.id ? { ...u, cancelled: true, speed: 0 } : u)))
        resolve()
      }
