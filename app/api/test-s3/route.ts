...
import { getConnections } from '@/app/(filemanager)/files/server/connection-actions';
import { S3Config } from '@/app/(filemanager)/files/types';
...
export async function GET() {
  ...
  // List with Delimiter
  const withDelim = await client.send(new ListObjectsCommand({
  ...
  const withoutDelim = await client.send(new ListObjectsCommand({
    Bucket: config.bucket,
    Prefix: 'test-folder/',
  }));
