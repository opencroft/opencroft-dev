...
async function resolveKeyContent(keyPath?: string): Promise<string | undefined> {
  ...
  if (parsed) {
    ...
    const candidates = [
      path.join(baseCache, 'local', 'core', 'key-store', parsed.storeId, parsed.name),
      path.join(baseCache, 'builtin', 'core', 'key-store', parsed.storeId, parsed.name),
    ];
...
export const sftp = {
  async list(creds: SshCredentials, dirPath: string): Promise<SftpEntry[]> {
    return withSftp(creds, (s) => new Promise((resolve, reject) => {
