...
import type { Anchor, Comment } from '@/app/(docs)/docs/_server/comments';
import { listDocComments, postDocComment } from '@/app/(docs)/docs/actions';
...
function renderMessage(message: string): React.ReactNode {
  ...
  while ((match = regex.exec(message)) !== null) {
    ...
    parts.push(
      <span key={`m-${i++}`} className="text-primary font-medium">@{match[1]}</span>,
    );
...
export function DocCommentsOverlay({ docPath, renderKey }: DocCommentsOverlayProps) {
  ...
  const selectedThread = openThread && comments
    ? comments.find(c => c.id === openThread) ?? null
    : null;
  ...
  return (
    <div ref={wrapperRef} className="absolute inset-0 pointer-events-none">
      ...
      {selection && !composerAnchor && typeof document !== 'undefined' && createPortal(
        <button
      ...
      {composerAnchor && typeof document !== 'undefined' && createPortal(
        <div
  ...
  return (
    <div className="flex flex-col gap-3 max-h-[70vh]">
      ...
      <div className="flex flex-col gap-2 max-h-60 overflow-y-auto pr-1">
        ...
        {replies.map(r => <CommentRow key={r.id} comment={r} indented />)}
      </div>
