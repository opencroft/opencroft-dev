...
import { loadGraph } from '@/app/(legacy-app-dashboard)/_legacy/app-dashboard/actions'
import { resolveServer } from '@/app/(legacy-app-dashboard)/_legacy/nodes/server/actions'
import { createDockerContext, detectOS, renameComposesFolder, renameDockerContext } from '@/app/(server)/_server/remote'
import { getDockerFeature, getSshFeature, type Server, type ServerFeature, slug } from '@/app/(server)/_server/types'
...
export const saveServer = createServerFn({ method: 'POST', strict: { output: false } })
  .inputValidator((data: { server: Server; oldSlug?: string }) => data)
...
export const deleteServer = createServerFn({ method: 'POST', strict: { output: false } })
