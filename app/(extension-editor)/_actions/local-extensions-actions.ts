...
import type { BuildResult, ExtensionManifest } from '@/app/(extension-runtime)/_types'
...
export const getLocalExtension = createServerFn({ method: 'POST', strict: { output: false } })
  ...
  .handler(async ({ data: extensionId }): Promise<LocalExtensionRecord | null> => {
...
export const updateLocalExtension = createServerFn({ method: 'POST', strict: { output: false } })
  ...
  .handler(async ({ data }): Promise<LocalExtensionRecord> => {
...
export const createLocalExtension = createServerFn({ method: 'POST', strict: { output: false } })
  ...
  .handler(async ({ data: files }): Promise<LocalExtensionRecord> => {
...
export const compileLocalExtension = createServerFn({ method: 'POST', strict: { output: false } })
  .inputValidator((extensionId: string) => extensionId)
  .handler(async ({ data: extensionId }): Promise<BuildResult> => {
    const slug = slugFromId(extensionId)
    const record = await loadExtension(slug)
    if (!record) {
      throw new Error(`Extension ${extensionId} does not exist`)
