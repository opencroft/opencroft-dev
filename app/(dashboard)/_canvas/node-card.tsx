...
export function NodeCard({ children, selected, loading, accent, error, className }: NodeCardProps) {
  ...
  const ringColor = error
    ? 'var(--destructive)'
    : accent ? `color-mix(in oklch, ${accent} 20%, transparent)` : undefined;
  return (
    <div
      className={cn(
  ...
  return (
    <div className={cn('flex items-center gap-2 px-4 py-2', className)}>
      ...
      <div className="relative shrink-0">
        ...
        {status && (
          <StatusIndicator variant={status} className="absolute -bottom-0.5 -right-0.5" />
        )}
      </div>
      <div className="flex flex-col min-w-0">
        <span className={cn('text font-medium truncate', titleClassName)}>{title}</span>
        {subtitle && (
          <span className="text-[10px] font-mono text-muted-foreground truncate">{subtitle}</span>
        )}
      </div>
