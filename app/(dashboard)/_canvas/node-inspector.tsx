...
export function NodeInspector({
  node, expanded, extensions, graphNodes, override,
  updateNodeData, onDeselect, onEditExtension, onNewExtension, onExpandedChange, onFocusNode,
}: NodeInspectorProps) {
...
function NodeBrowser({ extensions, graphNodes, onEditExtension, onFocusNode }: NodeBrowserProps) {
  ...
  return (
    <Flex expanded className="w-full h-full bg-card">
      ...
      <Separator />
      {tab === 'outline' ? (
        <OutlineTab graphNodes={graphNodes} onFocusNode={onFocusNode} />
      ) : (
        <PaletteTab extensions={extensions} onEditExtension={onEditExtension} />
      )}
    </Flex>
  );
}
