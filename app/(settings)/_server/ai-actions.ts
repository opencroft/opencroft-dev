...
import { getSetting, setSetting, updateSetting } from '@/app/(settings)/_server/actions'
...
export const saveDefaultAgent = createServerFn({ method: 'POST' })
  .inputValidator((agentId: string | null) => agentId)
  .handler(async ({ data: agentId }): Promise<AiSettings> => {
...
export const saveGatewayConfig = createServerFn({ method: 'POST' })
  .inputValidator((input: GatewayConfigInput) => input)
  .handler(async ({ data: input }): Promise<AiSettings> => {
    const patch = {
